...
function postprocess(processed, withHeader, bom, strictMode, swScopeHeaderValue, nativeAutomation, workerSettings) {
    ...
    if (withHeader)
        processed = (0, header_1.add)(processed, strictMode, swScopeHeaderValue, nativeAutomation, workerSettings);
...
function processScript(src, withHeader = false, wrapLastExprWithProcessHtml = false, resolver, swScopeHeaderValue, nativeAutomation, workerSettings) {
    ...
    const changes = nativeAutomation ? [] : (0, transform_1.default)(ast, wrapLastExprWithProcessHtml, resolver);
