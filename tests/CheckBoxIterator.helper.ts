...
export const clickThroughAllCheckboxes = async (t: TestController) => {
  const elementInfos = [
    {
      id: 'remote-testing',
  ...
  for (const elementInfo of elementInfos) await t.click(Selector(`#${elementInfo.id}`))

  const checkboxes = Selector('legend').withExactText('Which features are important to you:').parent('fieldset').child('input[type=checkbox]')
