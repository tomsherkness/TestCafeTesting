...
class DemoSiteHelper {
  ...
  ClickFeatureCheckBoxesAndVerifyLabels = async (t: TestController) => {
    const {
      remoteTestingCheckboxText,
      reusingJsCodeCheckboxText,
      parallelTestingCheckboxText,
      continuousIntegrationCheckboxText,
      markupAnalysisCheckboxText,
    } = featureImportanceCheckboxes
    await t.expect(this.remoteTestingCheckbox.checkboxText)
      .eql(remoteTestingCheckboxText)
    await t.expect(this.reusingJsCodeCheckbox.checkboxText)
      .eql(reusingJsCodeCheckboxText)
    await t.expect(this.parallelTestingCheckBox.checkboxText)
      .eql(parallelTestingCheckboxText)
    await t.expect(this.continuousIntegrationCheckbox.checkboxText)
      .eql(continuousIntegrationCheckboxText)
    await t.expect(this.markupAndAnalysisCheckbox.checkboxText)
      .eql(markupAnalysisCheckboxText)
  ...
  ClickOperatingSystemRadioOption = async (t: TestController, operatingSystem: string) => {
    const { windowsOsRadio, macOsRadio, linuxOsRadio } =
      operatingSystemRadioChoices
    ...
    if (operatingSystem === windowsOsRadio) {
      await this.windowsRadioButton.clickRadio(t)
    }
    if (operatingSystem === macOsRadio) {
      await this.macOsRadioButton.clickRadio(t)
    }
    if (operatingSystem === linuxOsRadio) {
      await this.linuxRadioButton.clickRadio(t)
    }
  }
  ...
  ClickSubmitAndVerifyThankYouHeader = async (t: TestController, developerName: string) => {
    ...
    await t.expect(getPageUrl()).contains('thank-you')
  }
  ...
  VerifyElementsDisabledByDefault = async (t: TestController) => {
    await t.expect(this.submitButton.withAttribute('disabled', 'disabled').exists)
      .ok('the submit button was not in a disabled state by default')
    await t.expect(this.textArea.selector.withAttribute('disabled', 'disabled').exists)
      .ok('the comment box area was not in a disabled state by default')
    await t.expect(this.disabledSlider.exists)
      .ok('the slider was not in a disabled state by default')
  }
}
